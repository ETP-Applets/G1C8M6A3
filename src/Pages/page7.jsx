...
const page7 = ({
  ...
  const getFeedbackMessage = () => {
    ...
    if (isCorrect === true && data.correctFeedbackArea) {
      ...
      return (
        <>
          {data.correctFeedbackArea.text1} <br/><br/> {data.correctFeedbackArea.text2} {verb} {difference} {data.correctFeedbackArea.missing} {missingAnimalName}. <br/><br/> {data.correctFeedbackArea.text3} {moreNameForSentence} {data.correctFeedbackArea.text4} {lessNameForSentence} {data.correctFeedbackArea.text5} {difference}.
        </>
      )
    }
    ...
    if (isCorrect === false && data.wrongFeedbackArea) {
      ...
      return (
        <>
          {data.wrongFeedbackArea.text1} {name1ForSentence} {data.wrongFeedbackArea.and} {name2ForSentence} {data.wrongFeedbackArea.text2}.<br/><br/> 
                ...
                >
                  {animalImage1 && number1 > 0 && (
                    <div className="row-outline-images">
                      ...
                      {/* Show empty boxes for missing images if this animal has fewer - only when correct answer is selected */}
                      {isCorrect === true && number1 < number2 && Array.from({ length: difference }).map((_, index) => (
                        <div
                          key={`empty-${index}`}
                          className="row-outline-image empty-box"
                        >
                          <div className="w-[4vw] h-[4vw] border-[0.45vh]  border-[#F9A942] rounded-[1vh]"></div>
                        </div>
                      ))}
                    </div>
                  )}
                ...
                >
                  {animalImage2 && number2 > 0 && (
                    <div className="row-outline-images">
                      ...
                      {/* Show empty boxes for missing images if this animal has fewer - only when correct answer is selected */}
                      {isCorrect === true && number2 < number1 && Array.from({ length: difference }).map((_, index) => (
                        <div
                          key={`empty-${index}`}
                          className="row-outline-image empty-box"
                        >
